...
/**
 * spring mvc 拦截器
* <p>Title: SystemHandlerInterceptorAdapter</p>
* <p>Description: </p>
...
public class SystemHandlerInterceptorAdapter extends HandlerInterceptorAdapter {
	...
	/**
	 * 预处理回调方法： 
	 * 在Controller方法前进行拦截 如果返回false
	 * 从当前拦截器往回执行所有拦截器的afterCompletion方法,再退出拦截器链. 如果返回true
	...
	//	HttpSession session = request.getSession(false);  //获取session，没有返回null
		System.out.println("--------------------"+uri+"-------------url-------"+url);
/*		
		//当不是资源文件夹
		if(uri.indexOf("/resources/")<0){
			
			Set<String> keySet =  CommonKey.getResourcesMap().keySet();
...
//		}
	...
	/**
	 * 后处理回调方法:
	 * 实现处理器的后处理（但在渲染视图之前），此时我们可以通过modelAndView（模型和视图对象）
